...
public class ElectricityBill {
    ...
    public static void main(String args[]) {
      ...
      try{
        ...
        int lastmonth=Integer.parseInt(sc.next());
        System.out.println("Enter this month units:");
        int presentmonth=Integer.parseInt(sc.next());
        int totalunits=lastmonth-presentmonth;
        double bill=0;
        if(totalunits<=50){
          bill=totalunits*0.5;
        }
        else if(totalunits<=150){
          bill=50*0.5+(totalunits-50)*1.75;
        }
        else if(totalunits<=250){
          bill=50*0.5+100*1.75+(totalunits-150)*2.5;
        }
        else{
          bill=50*0.5+100*1.75+100*2.5+(totalunits-250)*2.9;
        }
      bill=bill+0.1;
